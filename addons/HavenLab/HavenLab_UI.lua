...
local function BuildMain()
    ...
    local gm = Btn(left, "进战准备", 76, 22, function()
        HL:SendMany({ ".gm off", ".cheat god off" })
        HL:Print("已关 GM 和无敌。怪会打你。能用 .lab 的号会出黄字，不用开 GM。")
    end, "关掉 GM 模式和无敌，怪才会打你。调试黄字看账号 .lab 权限，不用开 GM。")
